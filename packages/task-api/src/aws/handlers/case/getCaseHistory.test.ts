...
const makeMockRepos = (overrides: {
  ...
  const caseHistoryRepo = {
    findByCaseId: vi.fn().mockResolvedValue(overrides.entries ?? []),
...
describe("getCaseHistory", () => {
  ...
  it("creator は history 一覧を取得できる", async () => {
    const { caseRepo, caseHistoryRepo, userRepo } = makeMockRepos({
      caseResult: baseCase,
      profileResult: mockProfile,
      entries: [mockEntry],
  ...
  it("TEAM target member は history を取得できる", async () => {
    const teamCase = {
      ...baseCase,
      creatorId: "other-user",
      ownerId: "other-user",
  ...
  it("history が存在しない場合は空配列を返す", async () => {
    const { caseRepo, caseHistoryRepo, userRepo } = makeMockRepos({
      caseResult: baseCase,
      profileResult: mockProfile,
      entries: [],
