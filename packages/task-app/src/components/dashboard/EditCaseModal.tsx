...
export const EditCaseModal: React.FC<EditCaseModalProps> = ({
  ...
  const [richDesc, setRichDesc] = useState<RichEditorOutput>({
    description: caseDetail.description,
    descriptionFormat: caseDetail.descriptionFormat ?? "plain",
    descriptionText: caseDetail.descriptionText ?? caseDetail.description,
  });
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 overflow-y-auto flex-1">
          ...
          <div>
            ...
            <RichEditor
              key={`${caseDetail.caseId}-${String(isOpen)}`}
              description={richDesc.description}
              descriptionFormat={richDesc.descriptionFormat}
