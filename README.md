...
- **v2.2.0（Dashboard 操作性改善・リッチエディタ・案件詳細 UI 刷新フェーズ）**: Dashboard の状態 pill UI とカードグリッド集中表示を追加し、CaseCard の情報密度を改善。あわせて権限付きの案件編集 UI、`PUT /cases/{id}` の title / description / dueDate 更新対応、Tiptap リッチエディタと private S3 presigned URL 画像アップロードを追加。さらに案件詳細画面を 2カラムレイアウトへ刷新。左カラムに案件本文＋タブ型コンテンツ（Tasks / Comments / History / Sub-cases）、右カラムに sticky サイドバー（ステータス更新・進捗バー・仕様・参加者・クレーム）を配置し、ヘッダー編集ボタン・パンくずリスト・案件ID コピーを追加した。
